...
import { Wallet, PieChart, TrendingUp, DollarSign, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export function BudgetWidget({ totalBudget = 5000, spent = 1200, currency = 'USD' }: Props) {
  const percentage = Math.min((spent / totalBudget) * 100, 100);
  const remaining = totalBudget - spent;

  const categories = [
    { label: 'Flights', amount: 800, percentage: 60, color: 'bg-red-500' },
    { label: 'Stay', amount: 300, percentage: 25, color: 'bg-blue-500' },
    { label: 'Food', amount: 100, percentage: 15, color: 'bg-green-500' },
  ];
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-3xl font-serif font-medium text-luxury-charcoal dark:text-white">
              {currency === 'USD' ? '$' : ''}{spent.toLocaleString()}
            </p>
          </div>
        ...
        <div className="space-y-3 mb-8">
          <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
            <span className="text-luxury-charcoal/50 dark:text-white/40">Utilization</span>
            <span className={cn(percentage > 90 ? 'text-red-500' : 'text-luxury-charcoal/70 dark:text-white/60')}>
          ...
          <div className="h-2 w-full bg-black/5 dark:bg-white/5 rounded-full overflow-hidden">
            <motion.div 
              ...
              className={cn("h-full rounded-full transition-all duration-1000", percentage > 90 ? 'bg-red-500' : 'bg-red-600')}
            />
          </div>
        </div>
          ...
          <div className="space-y-3">
            {categories.map((cat) => (
              <div key={cat.label} className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-luxury-charcoal dark:text-white">${cat.amount}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-black/5 dark:border-white/5 flex justify-between items-center">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-luxury-charcoal/40 dark:text-white/40">Remaining</p>
            <p className="text-lg font-serif font-medium text-luxury-charcoal dark:text-white">${remaining.toLocaleString()}</p>
          </div>
